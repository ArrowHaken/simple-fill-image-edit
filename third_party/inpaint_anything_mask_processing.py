"""Small vendored subset of Inpaint Anything crop/restore helpers.

The full upstream checkout is optional for the simple_fill lane.  Keeping the
geometry helpers here makes that lane reproducible while preserving the same
512px crop contract used by the original pipeline.
"""
...
def crop_for_filling_pre(image: np.ndarray, mask: np.ndarray, crop_size: int = 512):
    ...
    return image[crop_y:crop_y + crop_size, crop_x:crop_x + crop_size], mask[crop_y:crop_y + crop_size, crop_x:crop_x + crop_size]
